...
fcheck(Stud, Sub) :-
	...
	M > 2.
```
* При помощи `studsearch(Y, Stud, M)` (описан выше) находим оценку по предмету (Sub). Y - список-журнал оценок по предмету. Если эта оценка больше 2, то ученик (Stud) сдал экзамен (true). В другом случае - false. Так же можно при помощи автоматического перебора посмотреть, какие именно предметы сдал ученик.
...
'd' - необходим для рекурсии в случае сдачи ученика;

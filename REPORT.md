...
* `average(Stud, S)`: объединяет оценки студента в список, находит сумму элементов списка, длину списка, делит сумму на длину и выводит результат - средний бал ученика.  
...
'c' - вызов checkMax с хвостом списка (ведь учеников с максимальным средним баллом может быть несколько) и распечатка полученной головы - фамилии ученика;  
'd' - необходимо для рекурсии, если средний балл меньше максимального;  
* `gmax(N)`: объединяет средние баллы учеников определённой группы (N) в список (L) (`bagof(X, P, L)`), ищет максимальный средний балл из этого списка с помощью стандартного предиката `max_list(L, Max)` и вызывает  `checkMax(Y, Max)` со списком-группой (Y) и полученным максимальным баллом.
## Выводы

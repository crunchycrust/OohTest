...
К недостаткам реляционного представления можно отнести сложность и громоздкость правил, если предикаты-факты будут плохо подходить для решения конкретной задачи.  
...
Последнее условие - 'd' необходимо для рекурсивного обхода списка, если фамилии студентов не совпадают ('b' не выполняется).  
В итоге получаем: `studsearch(L, Stud, M)` <=> a U (b & c) U (~b & d);  
* `average(Stud, S)`: объединяет оценки студента в список, находит сумму элементов списка, длину списка, делит сумму на длину и выводит результат - средний балл студента.  

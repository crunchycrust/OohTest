...
Поиск в глубину обычно исследует сначала самую "левую" ветвь дерева, а когда процесс заходит в тупик, то он снова начинает из корня и идёт вниз по другому пути.  
```prolog
...
```
`df([Initial], Final, Res)` - предикат, осуществляющий сам поиск. `Now` - аргумент, в котором хранится растущая последовательность переходов. `Goal` - последовательность, являющаяся конечным сотоянием (она не изменяется в течении процесса и необходимо только для остановки процесса наращивания вариантов). `Res` -  переменная, в которую пойдёт конечный результат.  
...
roomBF(Initial, Final) :-
	...
	reverse(Res, Ref),	
    	...
    	write('TIME: '), write(T), nl, nl.
```
`bf([Now|BeforeNow], Goal, Res)` - предикат, осуществляющий сам поиск. `[Now|BeforeNow]` - аргумент, в котором хранятся все возможные растущие последовательности переходов. `Goal` - последовательность, являющаяся конечным сотоянием. `Res` -  переменная, в которую пойдёт конечный результат.  
...
roomI(Initial, Final) :-
	...
	write('TIME: '), write(T), nl, nl.
```
`id(Now, Goal, Res, Bound)` - предикат, осуществляющий сам поиск. `Now` - аргумент, в котором хранится растущая последовательность переходов. `Goal` - последовательность, являющаяся конечным сотоянием. `Res` -  переменная, в которую пойдёт конечный результат. `Bound` - граница.  
...
## Выводы
Из трёх вариантов поиска быстрее всех с задачей справился поиск с итеративным углублением. Что неудивительно, ведь такой способ комбинирует в себе меньший расход памяти из поиска в глубину и отсекание неподходящих вариантов из поиска в ширину. Это оптимальный поиск для нахождения кратчайшего решения, если у дерева есть конечная глубина. В ином случае желательно задавать максимальное ограничение, но тут повезло. И без максимального всё работает.  
  
Следующий по времени - поиск в ширину. Сработал медленнее, но тоже выдал кратчайший путь.  
Такой поиск подходит для тех задач, где ветви дерева состояний не имеют одинаковой длины. Он гарантирует нахождение кратчайшего пути. Однако при его использовании память может закончится очень быстро.  
  
А вот поиск в глубину стал последним. Это простейший метод поиска, но у него есть два главных недостатка:  
...
Пример второго пункта и виден в результате.  

Получаем, что оптимальным для решения данной задачи является поиск с итеративным погружением.  
  
Всё работает. Поиск в ширину для понимания в Прологе был немного тяжеловат. Работа была интересная.
Впрочем, этот отчёт писался дольше, чем для первой лабораторной. Очень долго.

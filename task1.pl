% „асть 1: ѕредикаты работы со списками

% 3. –еализаци€ стандартных предикатов обработки списков.
...
% ѕредикаты обработки списков.
% 4. ѕолучение последнего элемента (without basic predicates)
...
% 4. ѕолучение последнего элемента (with basic predicates)
% lastoo: if Z is the concatenation of sth and [X](list consisting only of X), then X is the last element. 
lastoo(Z, X) :- 
	append(_, [X], Z).

% ѕредикаты обработки числовых списков. 
% 5. ¬ычисление скал€рного произведени€ двух векторов-списков 
%(с учетом возможного несовпадени€ размерностей) (without basic predicates)
...
% 5. ¬ычисление скал€рного произведени€ двух векторов-списков 
%(с учетом возможного несовпадени€ размерностей) (with basic predicates)

scalar1(L1, L2, X) :-
	same_length(L1, L2),
	ssum(L1, L2, X).
	
% 6. —одержательный пример совместного использовани€ предикатов, реализованных в пунктах 3 и 4.
